...
def train_linear_regression(interval="1h", test_size=0.2):
    ...
    # Feature Engineering - align with ingestion.py columns
    df['Target'] = df['Close'].shift(-1)
    df = df.dropna()
    
    # Use standard pandas_ta column names from ingestion.py
    # RSI_14, SMA_20, SMA_50, EMA_20, MACD_12_26_9, BBL_20_2.0, BBU_20_2.0, ATR_14
    available_cols = df.columns.tolist()
    features = ['Close', 'Open', 'High', 'Low', 'Volume']
    
    # Add TA features if they exist
    ta_candidates = ['RSI_14', 'SMA_20', 'SMA_50', 'EMA_20', 'MACD_12_26_9', 'BBL_20_2.0', 'BBU_20_2.0', 'ATR_14']
    for col in ta_candidates:
    ...
    experiment_name = f"SPX500_LR_{interval}"
    ...
    with mlflow.start_run(run_name=f"LR_{interval}_Baseline"):
        ...
        mlflow_sklearn.log_model(model, "model", registered_model_name=f"SPX500_LR_{interval}")
...
if __name__ == "__main__":
    for interval in ["1h", "1d", "1w", "1m"]:
        train_linear_regression(interval=interval)
