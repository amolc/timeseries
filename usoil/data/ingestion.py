...
def download_usoil_data(interval="1h", period="60d", n_bars=2000):
    """
    Downloads historical USOIL data. Supports 1h, 1d, 1w, 1m intervals.
    """
    symbol = "USOIL"
    exchange = "TVC"
    ...
    try:
        ...
        if data is None or data.empty:
            ...
            return download_usoil_data_yfinance(interval=interval, period=period)
            
        # Standardize columns
        data.columns = [col.capitalize() for col in data.columns]
        
        os.makedirs(str(RAW_DIR), exist_ok=True)
        file_path = RAW_DIR / f"usoil_{interval}_raw.csv"
...
def download_usoil_data_yfinance(interval="1h", period="60d"):
    """
    Fallback: Downloads historical crude oil proxy data from Yahoo Finance.
    """
    symbol = "CL=F"
...
def preprocess_data(df, interval="1h"):
    ...
    processed_path = PROCESSED_DIR / f"usoil_{interval}_processed.csv"
...
def collect_all_intervals():
    ...
    for interval, params in intervals.items():
        raw_df = download_usoil_data(interval=interval, period=params['period'], n_bars=params['n_bars'])
...
if __name__ == "__main__":
    ...
    print("Multi-interval Data Collection Complete.")
