...
def paxusd_dashboard(request):
    ...
    latest_price = None
    latest_price_payload = get_last_price_payload("paxusd_dashboard", "PAXUSD", processed_file)
    ...
    latest_call = None
    for exp_name in ("PAXUSD_ARIMA_1h",):
    ...
    context['predictions_arima'] = _get_interval_predictions(
        client, "PAXUSD", "ARIMA", context['intervals'], latest_price=latest_price
    )
...
def last_price_api(request):
    processed_file = PROJECT_ROOT / "paxusd" / "data" / "processed" / "paxusd_1h_processed.csv"
    ticker = 'PAXUSD'
    response = get_last_price_payload("paxusd", ticker, processed_file)
