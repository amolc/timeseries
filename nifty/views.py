...
def nifty_dashboard(request):
    ...
    latest_price = None
    latest_price_payload = get_last_price_payload("nifty_dashboard", "NIFTY", processed_file)
    ...
    latest_call = None
    for exp_name in ("NIFTY_ARIMA_1h",):
    ...
    context['predictions_arima'] = _get_interval_predictions(
        client, "NIFTY", "ARIMA", context['intervals'], latest_price=latest_price
    )
...
def last_price_api(request):
    processed_file = PROJECT_ROOT / "nifty" / "data" / "processed" / "nifty_1h_processed.csv"
    ticker = 'NIFTY'
    response = get_last_price_payload("nifty", ticker, processed_file)
