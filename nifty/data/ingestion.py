...
def download_nifty_data(interval="1h", period="60d", n_bars=2000):
    """
    Downloads historical Nifty 50 data from NSE via TVDataFeed.
    Symbol: NIFTY
    Exchange: NSE
    """
    symbol = "NIFTY"
    ...
    try:
        ...
        if data is None or data.empty:
            ...
            return download_nifty_data_yfinance(interval=interval, period=period)
            
        # Standardize columns
        data.columns = [col.capitalize() for col in data.columns]
        
        os.makedirs(str(RAW_DIR), exist_ok=True)
        file_path = RAW_DIR / f"nifty_{interval}_raw.csv"
...
def download_nifty_data_yfinance(interval="1h", period="60d"):
    """
    Fallback: Downloads historical Nifty 50 data from Yahoo Finance.
    Symbol: ^NSEI
    """
    symbol = "^NSEI"
...
def preprocess_data(df, interval="1h"):
    ...
    if 'open' in cols_map:
        df['Open'] = df[cols_map['close']] # Using close as open if open is missing or handle it better
        if 'open' in cols_map: df['Open'] = df[cols_map['open']]
    if 'high' in cols_map:
    ...
    processed_path = PROCESSED_DIR / f"nifty_{interval}_processed.csv"
...
def collect_all_intervals():
    """
    Downloads and preprocesses data for all required intervals.
    """
    intervals = ["1h", "1d", "1w", "1m"]
    results = {}
    
    for interval in intervals:
        raw_data = download_nifty_data(interval=interval)
