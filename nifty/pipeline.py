...
def run_full_pipeline(skip_ingestion=False, models=("lr", "arima"), interval_filter=None):
    """
    Main pipeline entry point for Nifty intervals (1h, 1d, 1w, 1m).
    ...
    if not skip_ingestion:
        print(f"Starting Nifty data ingestion for intervals: {intervals}...")
    ...
    for interval in intervals:
        ...
        try:
            ...
            check_and_send_switchovers("NIFTY", interval)
        except Exception as e:
            print(f"Telegram notify failed: {e}")
            
    print(f"\nNifty Pipeline execution complete for {intervals}.")
