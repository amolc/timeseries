...
def run_full_pipeline(skip_ingestion=False, models=("lr", "arima"), interval_filter=None):
    """
    Main pipeline entry point for Gold intervals (1h, 1d, 1w, 1m).
    ...
    if not skip_ingestion:
        print(f"Starting Gold data ingestion for intervals: {intervals}...")
        # For now, collect_all_intervals might still do all, but we can refine later
        # To be efficient, we only need to ensure the data for the target interval exists
    ...
    for interval in intervals:
        print(f"\n--- Running models for Gold {interval} interval ---")
        interval_results = {}
        ...
        try:
            ...
            check_and_send_switchovers("GOLD", interval)
        except Exception as e:
            print(f"Telegram notify failed: {e}")
            
    print(f"\nGold Pipeline execution complete for {intervals}.")
    return pipeline_results

def parse_args():
    p = argparse.ArgumentParser(description="End-to-end multi-interval Gold pipeline")
