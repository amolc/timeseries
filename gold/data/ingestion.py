...
def download_gold_data(interval="1h", period="60d", n_bars=2000):
    """
    Downloads historical Gold data from TVC via TVDataFeed.
    Symbol: GOLD
    Exchange: TVC
    """
    symbol = "GOLD"
    ...
    try:
        ...
        if data is None or data.empty:
            ...
            return download_gold_data_yfinance(interval=interval, period=period)
            
        # Standardize columns
        data.columns = [col.capitalize() for col in data.columns]
        
        os.makedirs(str(RAW_DIR), exist_ok=True)
        file_path = RAW_DIR / f"gold_{interval}_raw.csv"
...
def download_gold_data_yfinance(interval="1h", period="60d"):
    """
    Fallback: Downloads historical Gold Futures data from Yahoo Finance.
    Symbol: GC=F
    """
    symbol = "GC=F"
...
def preprocess_data(df, interval="1h"):
    ...
    processed_path = PROCESSED_DIR / f"gold_{interval}_processed.csv"
...
def collect_all_intervals():
    ...
    for interval in intervals:
        raw_data = download_gold_data(interval=interval)
