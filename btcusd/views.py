...
def btcusd_dashboard(request):
    ...
    latest_price = None
    latest_price_payload = get_last_price_payload("btcusd_dashboard", "BTC-USD", processed_file)
    ...
    latest_call = None
    for exp_name in ("BTCUSD_ARIMA_1h",):
    ...
    context['predictions_arima'] = _get_interval_predictions(
        client, "BTCUSD", "ARIMA", context['intervals'], latest_price=latest_price
    )
...
def last_price_api(request):
    processed_file = PROJECT_ROOT / "btcusd" / "data" / "processed" / "btcusd_1h_processed.csv"
    ticker = 'BTC-USD'
    response = get_last_price_payload("btcusd", ticker, processed_file)
...
def _interval_detail(request, interval, model_override="ARIMA"):
    ...
    latest_price_payload = get_last_price_payload("btcusd_interval_detail", "BTC-USD", processed_file)
    if latest_price_payload.get("ok"):
        context["latest_price"] = latest_price_payload.get("latest_price", context["latest_price"])
    ...
    try:
        ...
        latest_call = None
        if comparison_data["ARIMA"]["signals"]:
            sig = comparison_data["ARIMA"]["signals"][0]
            # sig["time"] is string, we need to compare them or just take newest from MLflow again.
            # For simplicity, we'll re-check MLflow for the absolute newest across ARIMA for this interval.
            pass

        # Use the same logic as dashboard for header running P/L
        header_call = None
        for m in ["ARIMA"]:
            exp = client.get_experiment_by_name(f"BTCUSD_{m}_{interval}")
            if not exp: continue
        ...
        print(f"Error in BTCUSD _interval_detail: {e}")
